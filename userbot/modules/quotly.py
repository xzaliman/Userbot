...
async def quotess(qotli):
    ...
    if not qotli.reply_to_msg_id:
        return await qotli.edit("```Balas Ke Pesan```")
    ...
    if reply_message.sender.bot:
        return await qotli.edit("```Balas Ke Pesan```")
    await qotli.edit("```Sedang Memproses Sticker, Silahkan Menunggu```")
    try:
        async with bot.conversation(chat) as conv:
            try:
                ...
                return await qotli.reply("```Jangan Blockir @QuotLyBot Buka Blokir Lalu Coba Lagi```")
            if response.text.startswith("Hi!"):
                await qotli.edit("```Nonaktifkan Pengaturan Privasi Forward Anda```")
            else:
                await qotli.delete()
