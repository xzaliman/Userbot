...
DEF_UNAPPROVED_MSG = (
    ...
    "━━━━━━━━━━━━━━━━━━━━\n"
    f"__HALLO SELAMAT DATANG, MOHON JANGAN MELAKUKAN SPAM KARNA SAYA AKAN MEMBLOKIR ANDA, TUNGGU SAMPAI {DEFAULTUSER} MENERIMA PESAN ANDA__\n"
    "┏━━━━━━━━━━━━━━━━━━━\n"
    "┣[• `PESAN OTOMATIS`\n"
    "┣[• `BY USERBOT`\n"
    "┗━━━━━━━━━━━━━━━━━━━")
    ...
    if (
        ...
        if not apprv and event.text != UNAPPROVED_MSG:
            ...
            if COUNT_PM[event.chat_id] > 2:
                await event.respond(
                    "`You've been blocked and reported for spamming.`"
                )
...
async def disapprovepm(disapprvpm):
    ...
    await disapprvpm.edit(
        f"`Maaf` [{name0}](tg://user?id={disapprvpm.chat_id}) `Pesan Anda Ditolak, Mohon Jangan Melakukan Spam Ke Room Chat!`"
    )
    ...
    if event.is_private:
        if not pm_permit_sql.is_approved(chats.id):
            pm_permit_sql.approve(
                chats.id, "`Master A5N Telah Mengirimi Anda Pesan 😯`")
            await borg.send_message(
                chats, "**Menerima Pesan!, Pengguna Terdeteksi Adalah Master A5N**"
            )
