...
TOKEN_FILE_NAME = "GP_MASTER.json"
...
async def upload_google_photos(event):
    ...
    async with aiohttp.ClientSession() as session:
        headers = {
            "Content-Length": "0",
            "Master-Goog-Upload-Command": "start",
            "Master-Goog-Upload-Content-Type": mime_type,
            "Master-Goog-Upload-File-Name": file_name,
            "Master-Goog-Upload-Protocol": "resumable",
            "Master-Goog-Upload-Raw-Size": str(file_size),
        ...
        real_upload_url = step_one_resp_headers.get("Master-Goog-Upload-URL")
        logger.info(real_upload_url)
        upload_granularity = int(
            step_one_resp_headers.get("Master-Goog-Upload-Chunk-Granularity")
        )
        ...
        async with aiofiles.open(file_path, mode="rb") as f_d:
            for i in range(number_of_req_s):
                ...
                headers = {
                    ...
                    "Master-Goog-Upload-Command": "upload",
                    "Master-Goog-Upload-Offset": str(offset),
                    "Authorization": "Bearer " + creds.access_token,
                }
                logger.info(i)
                logger.info(headers)
            ...
            headers = {
                ...
                "Master-Goog-Upload-Command": "upload, finalize",
                "Master-Goog-Upload-Offset": str(number_of_req_s * upload_granularity),
                "Authorization": "Bearer " + creds.access_token,
            }
            logger.info(headers)
            response = await session.post(
